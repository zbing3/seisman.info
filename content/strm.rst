...
SRTM，全称为Shuttle Radar Topography
Mission，该项目获取了北纬60度至南纬60度之间的雷达影像数据，进一步处理制成了数字地形高程模型（DEM），即现在的SRTM地形产品数据。该数据产品2003年开始公开发布，经过多次修订，目前的数据修订版本为V4.1版本。更详细的介绍在\ `这里`_\ 。
...
-  网格划分采用gridline
   registration方式，因而单元的南北边及左右边与相邻单元的边重合。

数据信息 For 5x5
^^^^^^^^^^^^^^^^

部分网站将数据分割为5度\*5度单元，其相对1度\*1度单元使用起来免去了cut和paste的麻烦。在经度方向上，从-180度到180度，划分为72个区块；在纬度方向上，从60度到-60度，划分为24个区块，可以参考\ `这里`_\ 的图示，其文件名格式为“srtm\_经度区块数\_纬度区块数.zip”。
 或者也可以这样计算
...
一些问题
 ========
 - 中科院计算机网络中心国际科学数据服务平台网站目前无法访问，原因未知。

**国际科学数据服务平台：**\ `http://datamirror.csdb.cn/dem/search.jsp`_
 **版本：**\ V4和V4.1
...
**数据转换：**
 ...
 [code]xyz2grd srtm\_63\_02.asc -Gstrm\_63\_02.grd -E -V[/code]
 GeoTiff格式转netCDF格式（详情参考\ `这里`_\ ）：
 [code]gdal\_translate -of GMT srtm\_56\_05.tif srtm\_56\_05.grd[/code]

CGIAR-CSI
^^^^^^^^^

**网址：**\ `http://srtm.csi.cgiar.org/SELECTION/inputCoord.asp`_
 **版本：**\ 4.1
...
**网址：**\ `http://dds.cr.usgs.gov/srtm/`_
 **版本：**\ 1.0和2.1版本
 **格式：**\ 二进制z文件
...
这个例子选用的数据是N52E130.hgt，北纬、东经，-R=130/131/52/53，对于南纬或者西经要注意负号。-Z后使用TL是实践的结果，官方文档中没有给出相关信息。
 [code]xyz2grd N52E130.hgt -GN52E130.grd -I3c/3c -R130/131/52/53
-N-32768 -ZTLhw[/code]

USGS EarthExplorer
^^^^^^^^^^^^^^^^^^

**网址：**\ `http://earthexplorer.usgs.gov/`_
 **版本：**?.?
 **格式：**\ Geotiff、DTED、BIL
 **备注：**
...
**数据转换：**
 将BIL格式转换为GMT grid格式
 [code]xyz2grd n52\_e130\_3arc\_v1.bil -GN52E130.grd -I6c/3c -R$R
-N-32768 -ZTLh[/code]

CGIAR-CSI打包下载
^^^^^^^^^^^^^^^^^

CGIAR-CSI提供了ftp地址，支持所有数据的批量下载，数据总大小约14G。有需要的可以申请下载：\ `https://harvestchoice.wufoo.com/forms/download-cgiarcsi-srtm/`_

申请之后，网站会提供一个网址，以及用户名和密码，可以使用“Firefox+DownloadThemAll”下载，也可以使用如下命令下载：
 [code]wget -c -r -np --user=xxx --password=xxxxx
http://data.cgiar-csi.org/srtm/tiles[/code]

Box网盘下载
^^^^^^^^^^^

Box网盘中包含了重采样之后的数据，数据精度有250m、500m和1km三种，以满足其他人的需求。
 网盘地址：\ `https://hc.box.com/shared/1yidaheouv`_
 密码(注意感叹号)：ThanksCSI!
...
.. _这里: http://www.cgiar-csi.org/data/srtm-90m-digital-elevation-database-v4-1
.. _这里: http://srtm.csi.cgiar.org/SELECTION/inputCoord.asp
.. _`http://datamirror.csdb.cn/dem/search.jsp`: http://datamirror.csdb.cn/dem/search.jsp
.. _这里: http://seisman.info/convert-geotiff-to-gmt.html
.. _`http://srtm.csi.cgiar.org/SELECTION/inputCoord.asp`: http://srtm.csi.cgiar.org/SELECTION/inputCoord.asp
.. _`http://dds.cr.usgs.gov/srtm/`: http://dds.cr.usgs.gov/srtm/
.. _`http://earthexplorer.usgs.gov/`: http://earthexplorer.usgs.gov/
.. _`https://harvestchoice.wufoo.com/forms/download-cgiarcsi-srtm/`: https://harvestchoice.wufoo.com/forms/download-cgiarcsi-srtm/
.. _`https://hc.box.com/shared/1yidaheouv`: https://hc.box.com/shared/1yidaheouv
