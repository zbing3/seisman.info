...
#. 由于IRIS
   ftp里的数据默认会保存7天，且用户无删除权限，因而可能有些ftp中的数据已经下载了，所以在本地需要有一个已经下载的数据的列表local.filelist；
...
# get filelist of ftp
 ...
 &ftp\_list($server, $login, $passwd, $dir, $user);

# lists
 my @ftp\_list = (); # all the files in ftp
 my @local\_list = (); # files already downloaded
 my @finish\_list = (); # files already downloaded
 　　　　　　　　　　　　　# + files downloaded successfully in this
run.
...
##############################
 foreach (@ftp\_list) {
 my ($ftp\_file, $ftp\_size) = split /\\s+/, $\_;

# determine $ftp\_file downloaded or not
 my $in = 0;
 foreach (@local\_list) {
 chomp;
 if ($\_ =~ $ftp\_file) {
...
if ( $in==1 ) { # already downloaded
 push @finish\_list, $ftp\_file;
 } else { # need to be downloaded
 my $err = system "$cmd $option $server/$dir/$user/$ftp\_file";
 # err = 0 means succeed in downloading
 # err = 1 means downloading error
 # err = 2 means interrupt by user
 push @finish\_list, $ftp\_file if $err==0 && -s $ftp\_file ==
$ftp\_size;
 }
 }
 ########################## end downloading
